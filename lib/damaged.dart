...
import 'package:lab/login.dart'; // baseurl is imported from here

class DamagedPhonesScreen extends StatefulWidget {
  @override
  _DamagedPhonesScreenState createState() => _DamagedPhonesScreenState();
}

class _DamagedPhonesScreenState extends State<DamagedPhonesScreen> {
  List<dynamic> phones = [];
  bool isLoading = true;
  ...
  Future<void> fetchPhones() async {
    final dio = Dio();
    try {
      final response = await dio.get('$baseurl/ViewDamagedApi/$lid');
      ...
      print("Error fetching damaged phones: $e");
                    ...
                    child: Padding(
                      ...
                      child: Column(
                        ...
                        children: [
                          ...
                          SizedBox(height: 6),
                          Text("Condition: ${phone['status'] ?? 'N/A'}"),
                          ...
                          ElevatedButton(
                            onPressed: () {
                              Navigator.push(
                                ...
                                MaterialPageRoute(
                                  builder:
                                      (context) => DamagedPhoneDetailScreen(
                                        phoneData: phone,
                                      ),
                                ),
                              );
...
class DamagedPhoneDetailScreen extends StatelessWidget {
  final Map<String, dynamic> phoneData;

  DamagedPhoneDetailScreen({required this.phoneData});
          ...
          children: [
            ...
            Table(
              ...
              children: [
                ...
                buildTableRow('Serial No', phoneData['serial_no'] ?? ''),
                buildTableRow('Price', '₹${phoneData['price'] ?? ''}'),
                buildTableRow('Status', phoneData['status'] ?? ''),
